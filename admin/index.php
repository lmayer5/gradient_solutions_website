...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_settings') {
    ...
    $newSettings = [
        ...
        'western_university_text' => $_POST['western_university_text'] ?? '',
    ...
    <div class="container mx-auto p-6 max-w-6xl">
            ...
            <div x-show="open" class="mt-4 border-t pt-4" style="display: none;">
                <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="form-control">
                        ...
                        <input type="text" name="address" value="<?= htmlspecialchars($currentSettings['address']) ?>"
                            class="input input-bordered text-sm" required />
                    </div>

                    <div class="form-control">
                        <label class="label"><span class="label-text">Contact Email</span></label>
                        <input type="email" name="email" value="<?= htmlspecialchars($currentSettings['email']) ?>"
                            class="input input-bordered text-sm" required />
                    </div>

                    <div class="form-control">
                        <label class="label"><span class="label-text">Phone Number</span></label>
                        <input type="text" name="phone" value="<?= htmlspecialchars($currentSettings['phone']) ?>"
                            class="input input-bordered text-sm" required />
                    </div>

                    <div class="md:col-span-3 border-t pt-2 mt-4">
                        <h3 class="font-bold text-sm mb-2 text-brand-700"><i class="fa-brands fa-github mr-2"></i>GitHub
                            Delivery Settings</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
                            <div class="form-control">
                                <label class="label"><span class="label-text">GitHub Personal Access Token
                                        (PAT)</span></label>
                                <input type="password" name="github_pat"
                                    value="<?= htmlspecialchars($currentSettings['github_pat'] ?? '') ?>"
                                    class="input input-bordered text-sm" placeholder="ghp_xxx" />
                            </div>
                            <div class="form-control">
                                <label class="label"><span class="label-text">Default Repository
                                        (fallback)</span></label>
                                <input type="text" name="github_repo"
                                    value="<?= htmlspecialchars($currentSettings['github_repo'] ?? '') ?>"
                                    class="input input-bordered text-sm" placeholder="username/repository" />
                            </div>
                        </div>

                        <div class="mt-4 bg-gray-50 p-4 rounded-lg border border-dashed border-gray-300"
                            x-data="{ mappings: <?= htmlspecialchars(json_encode($currentSettings['repo_mappings'] ?? []), ENT_QUOTES, 'UTF-8') ?> }">
                            <h4 class="text-xs font-bold uppercase text-gray-500 mb-3 tracking-wider">Product-to-Repo
                                Mapping</h4>
                            <p class="text-[10px] text-gray-400 mb-3">Map specific Product IDs (found in catalog) to
                                repositories. If a product isn't mapped, the default repo above is used.</p>
                            <div class="space-y-2">
                                <template x-for="(map, index) in mappings" :key="index">
                                    <div class="flex gap-2 items-center">
                                        <input type="text" x-model="map.product_id"
                                            placeholder="Product ID (e.g. p_stem_gen)"
                                            class="input input-xs input-bordered flex-1" />
                                        <i class="fa-solid fa-arrow-right text-gray-300"></i>
                                        <input type="text" x-model="map.repo" placeholder="Repo (e.g. owner/repo)"
                                            class="input input-xs input-bordered flex-1" />
                                        <button type="button" @click="mappings.splice(index, 1)"
                                            class="btn btn-ghost btn-xs text-red-500"><i
                                                class="fa-solid fa-xmark"></i></button>
                                    </div>
                                </template>
                            </div>
                            <div class="mt-3">
                                <button type="button" @click="mappings.push({product_id: '', repo: ''})"
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div class="form-control">
                                ...
                                <input type="text" name="about_title"
                                    value="<?= htmlspecialchars($currentSettings['about_title'] ?? '') ?>"
                                    class="input input-bordered text-sm" />
                            </div>
                            <div class="form-control">
                                <label class="label"><span class="label-text">About Text</span></label>
                                <textarea name="about_text"
                                    class="textarea textarea-bordered h-24 text-sm"><?= htmlspecialchars($currentSettings['about_text'] ?? '') ?></textarea>
                            </div>
                        </div>
                    </div>

                    <div class="md:col-span-3 border-t pt-2 mt-2">
                        <h3 class="font-bold text-sm mb-2"><i class="fa-solid fa-university mr-2"></i>Western University
                            Section</h3>
                        <div class="grid grid-cols-1 gap-4">
                            <div class="form-control">
                                <label class="label"><span class="label-text">Western University Info
                                        Text</span></label>
                                <textarea name="western_university_text" class="textarea textarea-bordered h-24 text-sm"
                                    placeholder="Enter information about Western University..."><?= htmlspecialchars($currentSettings['western_university_text'] ?? '') ?></textarea>
                                <label class="label"><span class="label-text-alt text-gray-400">This text will appear in
                                        the Western University modal on the storefront.</span></label>
                            </div>
                        </div>
                    </div>

                    <div class="md:col-span-3 border-t pt-2 mt-2"
                        x-data="{ faqs: <?= htmlspecialchars(json_encode($currentSettings['faq'] ?? []), ENT_QUOTES, 'UTF-8') ?> }">
                            ...
                            <template x-for="(faq, index) in faqs" :key="index">
                                <div class="flex gap-2 items-start bg-gray-50 p-3 rounded-lg border">
                                    <div class="flex-1 grid gap-2">
                                        <input type="text" name="faq_questions[]" x-model="faq.question"
                                            placeholder="Question" class="input input-sm input-bordered w-full"
                                            required />
                                        <textarea name="faq_answers[]" x-model="faq.answer" placeholder="Answer"
                                            class="textarea textarea-sm textarea-bordered w-full" required></textarea>
                                    </div>
                                    <button type="button" @click="faqs.splice(index, 1)"
                                        class="btn btn-ghost btn-xs text-red-500 hover:bg-red-50"><i
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex items-center gap-4">
                <form method="POST"
                    onsubmit="return confirm('Are you sure you want to delete ALL orders? This cannot be undone.');">
                        ...
                        <?php foreach ($orders as $order): ?>
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors" x-data="{ 
                                ...
                                <td class="py-3 px-6">
                                    <div class="flex flex-col">
                                        <div class="font-medium text-gray-800">
                                            <?= htmlspecialchars($order['customer']['name']) ?></div>
                                        <div class="text-xs text-gray-400 mb-1">
                                            <?= htmlspecialchars($order['customer']['email']) ?></div>
                                        <?php if (!empty($github)): ?>
                                            <div class="text-[10px]">
                                                <a href="https://github.com/<?= htmlspecialchars($github) ?>" target="_blank"
                                ...
                                <td class="py-3 px-6 text-right">
                                    <a x-show="status === 'Completed' || status === 'Shipped' || true"
                                        href="<?= isset($order['invoice_file']) ? '../view_invoice.php?file=' . htmlspecialchars($order['invoice_file']) : '#' ?>"
                                        target="_blank" class="btn btn-xs btn-outline btn-info mr-2"
                                        :class="{ 'btn-disabled': !'<?= isset($order['invoice_file']) ? $order['invoice_file'] : '' ?>' }">
                                        <i class="fa-solid fa-file-pdf"></i> PDF
                                    </a>
                                    <button x-show="status === 'Pending'" @click="processDelivery()" :disabled="isLoading"
                                        class="btn btn-xs btn-outline btn-success">
                                    ...
                                    <button x-show="status !== 'Pending'" @click="inviteToRepo()"
                                        :disabled="isInviting || !github" class="btn btn-xs btn-outline btn-primary ml-1">
